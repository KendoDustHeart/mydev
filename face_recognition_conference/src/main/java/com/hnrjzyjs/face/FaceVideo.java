...
public class FaceVideo {
    ...
    public static void main(String[] args) {
        ...
        String basePicPath = "Pictures/";
        double compareHist = compare_image(basePicPath + "pyy1.jpg", basePicPath + "pyy2.jpg");
...
//        CascadeClassifier facebook=new CascadeClassifier("D:\\Sofeware\\opencv\\sources\\data\\haarcascades\\haarcascade_frontalface_alt.xml");
        ...
        if(rects != null && rects.length >= 1) {
            ...
            if(i==3) {// 获取匹配成功第10次的照片
                Imgcodecs.imwrite("Pictures/" + "face.png", image);
            }
        }
        return image;
    }
...
//        CascadeClassifier facebook=new CascadeClassifier("D:\\Sofeware\\opencv\\sources\\data\\haarcascades\\haarcascade_frontalface_alt.xml");
        // 2 读取测试图片
        String imgPath = "Pictures/pyy2.jpg";
        ...
        for (Rect rect : face.toArray()) {
            ...
            imageCut(imgPath, "images/"+i+".jpg", rect.x, rect.y, rect.width, rect.height);// 进行图片裁剪
            i++;
        }
        // 6 展示图片
    ...
    public static void writeVideo() {
        ...
        while(capture.isOpened()) {
            ...
            writer.write(video);
            HighGui.imshow("摄像头获取视频", video);//3 显示图像
            index=HighGui.waitKey(100);//4 获取键盘输入
            if(index==27) {//5 如果是 Esc 则退出
                capture.release();
                writer.release();
